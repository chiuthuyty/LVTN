...
import { AuthReponse, RefreshTokenReponse } from 'src/types/auth.type'
import {
  clearLS,
  ...
  constructor() {
    ...
    this.instance.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        //Lỗi Unathorized(401) có rất nhiều trường hợp
        //-Token không đúng
        //-Không truyền token
        //-Token hết hạn
        ...
        if (isAxiosUnauthorizedError<ErrorResponse<{ name: string; message: string }>>(error)) {
          ...
          if (isAxiosExpiredTokenError(error) && url !== URL_REFRESH_TOKEN) {
            ...
            return this.refreshTokenRequest.then((access_token) => {
              ...
              return this.instance({ ...config, headers: { ...config.headers, authorzation: access_token } })
            })
          }
